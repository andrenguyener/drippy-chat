...
func (ctx *Context) UsersHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		// ctx.TrieStore.Add(strings.ToLower(user.Email), user.ID)
		// ctx.TrieStore.Add(strings.ToLower(user.UserName), user.ID)
		// ctx.TrieStore.Add(strings.ToLower(user.FirstName), user.ID)
		// ctx.TrieStore.Add(strings.ToLower(user.LastName), user.ID)
	...
	case "GET":
		if len(r.Header.Get("Authorization")) == 0 {
			...
			if len(searchPrefix) == 0 {
				...
				searchResults := ctx.TrieStore.DFSChildren(strings.ToLower(searchPrefix), 20)
				users := ctx.UserStore.ReturnUsers(searchResults)
...
func (ctx *Context) UsersMeHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "PATCH":
		...
		firstNameDel := strings.ToLower(sessionUser.FirstName)
		lastNameDel := strings.ToLower(sessionUser.LastName)
		...
		ctx.TrieStore.Delete(firstNameDel, sessionUser.ID)
		ctx.TrieStore.Delete(lastNameDel, sessionUser.ID)
		ctx.TrieStore.Add(strings.ToLower(userUpdates.FirstName), sessionUser.ID)
		ctx.TrieStore.Add(strings.ToLower(userUpdates.LastName), sessionUser.ID)
